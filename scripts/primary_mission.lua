...
function script_load(settings)
    hotkey_id = Obslua.obs_hotkey_register_frontend(
        "change_primary_mission_hotkey",
        "[From Lua Script] Primary Mission",
        on_hotkey
    )

    local hotkey_array = Obslua.obs_data_get_array(settings, "change_primary_mission_hotkey")
    Obslua.obs_hotkey_load(hotkey_id, hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end

function script_description()
    return "Primary Mission – Changes the Primary Mission in data/primary_mission.txt"
end

function script_save(settings)
    local hotkey_array = Obslua.obs_hotkey_save(hotkey_id)
    Obslua.obs_data_set_array(settings, "change_primary_mission_hotkey", hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end