...
function script_load(settings)
    hotkey_id = Obslua.obs_hotkey_register_frontend(
        "change_attacker_secondary_hotkey",
        "[From Lua Script] Attacker Secondary",
        on_hotkey
    )

    local hotkey_array = Obslua.obs_data_get_array(settings, "change_attacker_secondary_hotkey")
    Obslua.obs_hotkey_load(hotkey_id, hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end

function script_description()
    return "Attacker Secondary – Changes the Attacker's Secondary in data/attacker_secondary.txt"
end

function script_save(settings)
    local hotkey_array = Obslua.obs_hotkey_save(hotkey_id)
    Obslua.obs_data_set_array(settings, "change_attacker_secondary_hotkey", hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end