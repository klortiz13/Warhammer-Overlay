...
local file_path = "C:/Users/leona/OneDrive/Documents/Code/Warhammer_OBS_Sources/data/defender_secondary.txt"
...
local function increment_round()
    ...
    index = (index < size) and (index + 1) or 1
...
function script_load(settings)
    hotkey_id = Obslua.obs_hotkey_register_frontend(
        "change_defender_secondary_hotkey",
        "[From Lua Script] Defender Secondary",
        on_hotkey
    )

    local hotkey_array = Obslua.obs_data_get_array(settings, "change_defender_secondary_hotkey")
    Obslua.obs_hotkey_load(hotkey_id, hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end

function script_description()
    return "Defender Secondary – Changes the Defender's Secondary in data/defender_secondary.txt"
end

function script_save(settings)
    local hotkey_array = Obslua.obs_hotkey_save(hotkey_id)
    Obslua.obs_data_set_array(settings, "change_defender_secondary_hotkey", hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end