...
function script_load(settings)
    ...
    local hotkey_array = Obslua.obs_data_get_array(settings, "increment_attacker_cp_hotkey")
    Obslua.obs_hotkey_load(hotkey_id, hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end

function script_description()
    return "Attacker CP – increments the Attacker's CP in data/attacker_CP.txt"
end

function script_save(settings)
    local hotkey_array = Obslua.obs_hotkey_save(hotkey_id)
    Obslua.obs_data_set_array(settings, "increment_attacker_cp_hotkey", hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end