...
function script_load(settings)
    hotkey_id = Obslua.obs_hotkey_register_frontend(
        ...
        "[From Lua Script] Defender Secondary Down",
        on_hotkey
    )

    local hotkey_array = Obslua.obs_data_get_array(settings, "change_defender_secondary_down_hotkey")
    Obslua.obs_hotkey_load(hotkey_id, hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end

function script_description()
    return "Defender Secondary Down – Changes the Defender's Secondary Down in data/defender_secondary.txt"
end

function script_save(settings)
    local hotkey_array = Obslua.obs_hotkey_save(hotkey_id)
    Obslua.obs_data_set_array(settings, "change_defender_secondary_down_hotkey", hotkey_array)
    Obslua.obs_data_array_release(hotkey_array)
end